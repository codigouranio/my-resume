...
class ResumeCastACPAgent {
  ...
  setupRoutes() {
    ...
    this.app.post("/create-resume", async (req, res) => {
      try {
        ...
        const result = await this.createResumeWithAI(
          userInput,
          jobDescription,
          targetRole,
        );
    ...
    this.app.post("/optimize-resume/:resumeId", async (req, res) => {
      try {
        ...
        const result = await this.optimizeResumeWithAI(
          resumeId,
          jobDescription,
        );
        res.json(result);
  ...
  async executeTool(tool, params) {
    ...
    switch (tool) {
      case "create_resume":
        return await this.createResumeWithAI(
          params.userInput,
      ...
      case "optimize_resume":
        return await this.optimizeResumeWithAI(
          params.resumeId,
          params.jobDescription,
        );
  ...
  async createResumeWithAI(userInput, jobDescription, targetRole) {
    ...
    try {
      ...
      const resumeResponse = await axios.post(
        `${this.resumecastUrl}/api/resumes`,
        {
      ...
      const atsScore = await this.calculateATSScore(
        resumeContent,
        jobDescription,
      );
      ...
      return {
        ...
        atsScore,
        keywordMatch: await this.extractKeywordMatch(
          resumeContent,
          jobDescription,
        ),
  ...
  async optimizeResumeWithAI(resumeId, jobDescription) {
    ...
    try {
      ...
      const resumeResponse = await axios.get(
        `${this.resumecastUrl}/api/resumes/${resumeId}`,
      );
  ...
  async generateOptimizationSuggestions(resumeContent, jobDescription) {
    ...
    if (resumeContent.length < 500) {
      suggestions.push(
        "Consider adding more detail to your experience section",
      );
    }
  ...
  start() {
    ...
    this.app.listen(3001, () => {
      console.log(
        "🚀 ResumeCast ACP Agent HTTP server on http://localhost:3001",
      );
